			...
			if typ == "string" then
				...
				elseif getsynasset and #image > 14 and string.sub(image, 1, 14) == "synasseturl://" then
					local x, e = pcall(function()
						...
						local fold = isfolder("./FUCKPF")
						if not fold then
							makefolder("./FUCKPF")
						end
						fold = isfolder("./FUCKPF/Themes")
						if not fold then
							makefolder("./FUCKPF/Themes")
						end
						fold = isfolder("./FUCKPF/Themes/SynapseAssetsCache")
						if not fold then
							makefolder("./FUCKPF Themes/SynapseAssetsCache")
						end
						if not fold or not isfile("./FUCKPF/Themes/SynapseAssetsCache/" .. codename .. ".dat") then
							local res = game:HttpGet(string.sub(image, 15))
							if res ~= nil then
								writefile("./FUCKPF/Themes/SynapseAssetsCache/" .. codename .. ".dat", res)
							end
						end
						return getsynasset(readfile("./FUCKPF/Themes/SynapseAssetsCache/" .. codename .. ".dat"))
					end)
					if x and e ~= nil then
						return e
					end
...
function library:CreateWindow(options)
	...
	if windowName and #windowName > 0 and library.WorkspaceName == "FUCKPF" then
		...
		function tabFunctions:CreateSection(options)
			...
			if isfolder and makefolder and listfiles and readfile and writefile then
				sectionFunctions.AddPersistence = function(_, options)
					...
					local realDropdownHolderList = Instance_new("UIListLayout")
					local dropdownEnabled = false
					if not isfolder("./FUCKPF") then
						makefolder("./FUCKPF")
					end
					local common_string = "./FUCKPF/" .. tostring(custom_workspace or library.WorkspaceName)
					local function resolvelist()
						if custom_workspace ~= options.Workspace then
							custom_workspace = options.Workspace
							common_string = "./FUCKPF/" .. tostring(custom_workspace or library.WorkspaceName)
						end
